...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/osdate/Neverlose/'..readfile('newvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
...
function mainapi:CreateCategory(categorysettings)
	...
	categoryapi.Children = window
	categoryapi.Type = 'Tab'
...
function mainapi:CreateNotification(title, text, duration, type)
	print(title, text, duration, type)
end
	...
	if isfile('newvape/profiles/'..game.GameId..'.gui.txt') then
		...
		if not skipgui then
			...
			for i, v in guidata.Categories do
				...
				if not object or object.Type ~= 'Overlay' and object.Type ~= 'CategoryTheme' and object.Type ~= 'Tab' then continue end
				if v.Expanded then
					object:Expand(true)
				end
				if v.Position then
...
function mainapi:Save(newprofile)
	...
	for i, v in self.Categories do
		if v.Type ~= 'Overlay' and v.Type ~= 'CategoryTheme' and v.Type ~= 'Tab' then continue end
		warn(i, v, "ye saved im kanye yo")
		guidata.Categories[i] = {
			Position = v.Children and {X = v.Children.Position.X.Offset, Y = v.Children.Position.Y.Offset} or nil,
			Expanded = v.Expanded or false
		}
	end
	print('broo its done')
...
function mainapi:UpdateTextGUI(afterload)
	...
	for i, v in mainapi.Modules do
		if v.Enabled or table.find(found, i) then
			local moduletext = i..(v.ExtraText and " <font color='rgb(200, 200, 200)'>"..v.ExtraText()..'</font>' or '')
			local size = getfontsize(removeTags(moduletext), 14, uipallet.FontSemiBold)
			local background = Instance.new('Frame')
			background.Name = i
			background.BackgroundTransparency = 1
			background.BorderSizePixel = 0
			background.BackgroundColor3 = Color3.new()
			background.Size = UDim2.fromOffset((size.X + 12), 20)
			background.Parent = holder
			local offset = (background.Size.X.Offset + 4)
			local mainframe = background:Clone()
			mainframe.BackgroundTransparency = 0.4
			mainframe.Position = UDim2.fromOffset(offset, 0)
			mainframe.Parent = background
			local label = Instance.new('TextLabel')
			label.BackgroundTransparency = 1
			label.Text = moduletext
			label.RichText = true
			label.FontFace = uipallet.FontSemiBold
			...
			side.BackgroundColor3 = uipallet.Main
			side.Position = UDim2.fromOffset(-4, 0)
			side.Size = UDim2.new(0, 4, 1, 0)
			side.Parent = mainframe
			local corner = addCorner(side, UDim.new(1, 0))
			corner.TopRightRadius = UDim.new()
			corner.BottomRightRadius = UDim.new()
			local f = table.find(found, i)
			if not v.Enabled and f then
				mainframe.Position = UDim2.fromOffset()
				local z = tween:Tween(mainframe, info, {
					Position = UDim2.fromOffset(offset, 0)
				})
				z.Completed:Once(function()
					self:UpdateTextGUI()
				end)
			else
				if v.Enabled and not f then
					tween:Tween(mainframe, info, {
						Position = UDim2.fromOffset(0, 0)
					})
				else
					mainframe.Position = UDim2.fromOffset(v.Enabled and 0 or offset, 0)
				end
			end
	...
	table.sort(VapeLabels, function(a, b)
		return a.Object.Size.X.Offset > b.Object.Size.X.Offset
	end)
...
watermark.AnchorPoint = Vector2.new(0.01, 0.07)
watermark.Position = UDim2.fromScale(0.01, 0.07)
watermark.Size = UDim2.fromOffset(165, 62)
watermark.Image = getcustomasset('newvape/assets/liquidbounce/logo.png')
Instance.new('UIScale', watermark).Scale = 1.1
...
do
	...
	holder.Size = UDim2.fromOffset(220, 200)
	holder.Position = UDim2.fromScale(1, 0)
	holder.Parent = gui
	local sorter = Instance.new('UIListLayout')
	sorter.HorizontalAlignment = Enum.HorizontalAlignment.Right
...
mainapi.Legit = setmetatable(mainapi.Categories.Render, {
	__index = function(_, index)
		return rawget(mainapi, index)
	end
})
mainapi.Categories.Main = mainapi.Categories.Minigames:CreateModule({
	Name = 'Settings',
